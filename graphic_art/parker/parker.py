...
class Parker:
    ...
    def write_name(self, x, y, bVertical, box_size, border):
        ...
        for index, ch in enumerate(self.name):
            ...
            self.dwg.add(self.dwg.text(ch, insert=(x_pos+offset, y_pos+offset),
                                                    ...
                                                    fill='white', 
                                                    text_anchor="middle", 
                                                    dy=['0.3em'])) #dominant_baseline="central"
    ...
    def generate(self):
        self.name = 'PARKER'
        self.colors = [self.color_str(222,45,38)]
        self.colors.append(self.color_str(55,126,184))
        self.colors.append(self.color_str(77,175,74))
        self.colors.append(self.color_str(152,78,163))
        self.colors.append(self.color_str(255,127,0))
        self.colors.append(self.color_str(77,175,74))
        ...
        # signature
        self.dwg.add(self.dwg.text("dhp 2015", x=[(self.canvasWidth-20)], y=[(self.canvasHeight-20)], font_size=24, fill=self.color_str(64,64,64), text_anchor="end",))
            
        self.dwg.save()
        
if __name__ == "__main__":
    output_file = './images/parker.svg'
    parker = Parker(output_file)
    parker.generate()
    
    os.system('"C:\Program Files\Inkscape\inkscape.exe" -z -e %s -w %d -h %d %s' % ('./images/parker.png', parker.canvasWidth, parker.canvasHeight, output_file))
